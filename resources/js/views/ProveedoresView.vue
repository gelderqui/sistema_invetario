...
async function save() {
  ...
  try {
    ...
    if (editingId.value) {
      const { data } = await axios.put(`/catalogos/proveedores/${editingId.value}`, payload);
      const idx = proveedores.value.findIndex((x) => x.id === editingId.value);
      if (idx !== -1) proveedores.value[idx] = data.data;
    } else {
      const { data } = await axios.post('/catalogos/proveedores', payload);
...
async function confirmToggle() {
  toggling.value = true;
  try {
    const { data } = await axios.patch(`/catalogos/proveedores/${selected.value.id}/toggle`);
...
async function confirmDelete() {
  deleting.value = true;
  try {
    await axios.delete(`/catalogos/proveedores/${selected.value.id}`);
